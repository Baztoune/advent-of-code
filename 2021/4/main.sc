...
case class Board(values: Seq[Seq[Int]]) {
  def hasWon(numbers: Seq[Int]) = {
    values.exists(_.forall(numbers.contains)) ||
      values.transpose.exists(_.forall(numbers.contains))
  }

  def score(numbers: Seq[Int]) = {
    val unmarked = values.flatten.filterNot(numbers.contains).sum
    val lastNumber = numbers.lastOption.getOrElse(0)
...
object Board {
  def apply(input: String): Board = {
    val boardValues = input.split('\n').toSeq.map(_.trim.split("[^\\d]+").toSeq.map(_.toInt))
    Board(boardValues)
  }
}
...
object Game {
  def apply(input: String): Game = {
    val (numbers, boards) = input.split("\n\n").toSeq match {
      case firstBlock +: nextBlocks =>
        val numbers = firstBlock.split(",").toSeq.map(_.toInt)
        (numbers, nextBlocks.map(Board(_)))
    }
    Game(numbers, boards)
  }
}
